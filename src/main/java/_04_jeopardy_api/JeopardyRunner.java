...
public class JeopardyRunner {
	...
	public static void main(String[] args) {
		...
		for (int i = 100; i <= 1000; i += 100) {
			...
			JeopardyApi j = new JeopardyApi();
			String question = j.getClue(i).getQuestion();
			String answer = j.getClue(i).getAnswer();
			System.out.println(answer);
			String title = j.getClue(i).getCategory().getTitle();
			String input = JOptionPane.showInputDialog(null, question);
			if (input.equalsIgnoreCase(answer)) {
				score += j.getClue(i).getValue();
			}
		}
